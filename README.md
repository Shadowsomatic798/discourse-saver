# LinuxDo to Obsidian V3.5.7

**劫持LinuxDo链接按钮**，一键保存帖子+评论到 **Obsidian** 或 **飞书多维表格**，支持双保存。
...
- **劫持链接按钮**：单击保存到 Obsidian/飞书，双击复制链接
- **评论链接支持**：点击评论区的链接按钮，保存主帖+该评论（文件名带楼层号）
...
| **单击** 评论链接按钮 | 保存主帖 + 该评论（文件名带楼层号） |
| **双击** 任意链接按钮 | 复制链接（必须是同一个按钮） |
...
**A:** V3.5.4 已修复此问题。请确保双击的是**同一个**链接按钮。快速点击**不同的**链接按钮不会触发双击。
